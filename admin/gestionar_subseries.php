...
<head>
    ...
    <title>Gestionar Subseries - Doroti</title>
...
<body>
    ...
    <nav class="navbar fixed-top" style="background-color: #e3f2fd;" data-bs-theme="light">
        <div class="container-fluid">
            <a class="navbar-brand d-flex align-items-center" href="admin.php">
                ...
                <span class="ms-2 fw-bold text-primary">ADMIN - Subseries</span>
            </a>
    ...
    <main class="container mt-5 pt-5">
        <h1 class="mb-4"><i class="bi bi-folder2-open text-primary"></i> Gestionar Subseries</h1>

        <!-- Área de mensajes -->
        <div id="messageArea"></div>

        <!-- Formulario para agregar/editar subserie -->
        <div class="form-section">
            ...
            <form id="formSubserie">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 mb-3">
                        <label for="serie_id" class="form-label">Serie Documental <span class="text-danger">*</span></label>
                        <select class="form-select" id="serie_id" name="serie_id" required disabled>
                            <option value="">Seleccione una dependencia primero...</option>
                            <!-- Se llenará dinámicamente según la dependencia -->
                        </select>
                    </div>
                    <div class="col-md-4 mb-3">
                        <label for="nombre" class="form-label">Nombre de Subserie <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" id="nombre" name="nombre" required>
                    </div>
                </div>
                <div class="d-flex gap-2 mt-2">
                    <button type="submit" class="btn btn-primary" id="btnSubmit">
                        <i class="bi bi-plus-lg me-1"></i><span id="btnSubmitText">Agregar Subserie</span>
                    </button>
        ...
        <div class="card">
            <div class="card-header bg-primary text-white">
                <h4 class="mb-0"><i class="bi bi-list-ul"></i> Lista de Subseries</h4>
            </div>
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-striped table-hover" id="tablaSubseries">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>¿Está seguro de que desea eliminar la subserie <strong id="nombreEliminar"></strong>?</p>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Event listener para cambiar de dependencia
            document.getElementById('dependencia_id').addEventListener('change', function() {
                ...
                if (depId) {
                    ...
                    serieSelect.innerHTML = '<option value="">Seleccione una dependencia primero...</option>';
                }
            });
        });
        ...
        // Cargar series por dependencia
        function cargarSeries(dependenciaId, selectedSerieId = null) {
            fetch(`api_subseries.php?action=list_series&dependencia_id=${dependenciaId}`)
                .then(response => response.json())
                .then(data => {
                    const serieSelect = document.getElementById('serie_id');
                    if (data.success) {
                        const optionsHtml = data.data.map(serie =>
                            `<option value="${serie.id}">${escapeHtml(serie.nombre)}</option>`
                        ).join('');
                        serieSelect.innerHTML = '<option value="">Seleccionar Serie...</option>' + optionsHtml;
                        serieSelect.disabled = false;

                        if (selectedSerieId) {
                            serieSelect.value = selectedSerieId;
                        }
                    } else {
                ...
                .catch(error => {
                    const serieSelect = document.getElementById('serie_id');
                    serieSelect.innerHTML = '<option value="">Error de conexión</option>';
                    mostrarMensaje('Error de conexión al cargar series: ' + error.message, 'danger');
                });
        }
        ...
        // Cargar lista de subseries
        function cargarSubseries() {
            fetch('api_subseries.php?action=list')
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        subseriesData = data.data;
                        renderizarTabla(data.data);
                    } else {
                        mostrarMensaje('Error al cargar subseries: ' + data.message, 'danger');
                    }
                })
                .catch(error => {
        ...
        // Renderizar tabla
        function renderizarTabla(subseries) {
            const tbody = document.getElementById('tbodySubseries');

            if (subseries.length === 0) {
                tbody.innerHTML = '<tr><td colspan="5" class="text-center text-muted">No hay subseries registradas</td></tr>';
                return;
            }
            ...
            <tr>
                ...
                <td><span class="badge bg-secondary">${escapeHtml(sub.serie_nombre || 'N/A')}</span></td>
                <td><span class="badge bg-info text-dark">${escapeHtml(sub.dependencia_nombre || 'N/A')}</span></td>
                <td class="table-actions">
                    <button class="btn btn-sm btn-warning" onclick="editarSubserie(${sub.id})" title="Editar">
                        <i class="bi bi-pencil"></i>
                    </button>
                    <button class="btn btn-sm btn-danger" onclick="mostrarModalEliminar(${sub.id}, '${escapeHtml(sub.nombre)}')" title="Eliminar">
        ...
        // Agregar o actualizar subserie
        document.getElementById('formSubserie').addEventListener('submit', function(e) {
            e.preventDefault();
        ...
        function editarSubserie(id) {
            const sub = subseriesData.find(s => s.id == id);
            if (!sub) return;

            // Llenar formulario
            document.getElementById('subserieId').value = sub.id;
            document.getElementById('nombre').value = sub.nombre;
            document.getElementById('dependencia_id').value = sub.dependencia_id;

            // Cargar series para la dependencia actual y seleccionar la serie_id
            cargarSeries(sub.dependencia_id, sub.serie_id);

            // Cambiar a modo edición
            modoEdicion = true;
            document.getElementById('formTitle').innerHTML = '<i class="bi bi-pencil-square"></i> Editar Subserie';
            document.getElementById('btnSubmitText').textContent = 'Actualizar Subserie';
        ...
        function resetearFormulario() {
            document.getElementById('formSubserie').reset();
            document.getElementById('subserieId').value = '';
            document.getElementById('serie_id').innerHTML = '<option value="">Seleccione una dependencia primero...</option>';
            document.getElementById('serie_id').disabled = true;
            modoEdicion = false;
            document.getElementById('formTitle').innerHTML = '<i class="bi bi-plus-circle"></i> Agregar Nueva Subserie';
            document.getElementById('btnSubmitText').textContent = 'Agregar Subserie';
