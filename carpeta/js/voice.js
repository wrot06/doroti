(function() {
  ...
  const mapaReemplazos = (Array.isArray(window.REEMPLAZOS) ? window.REEMPLAZOS : []).reduce((m, [k,v]) => {
    m.set(normalizarTexto(k), v); return m;
  }, new Map());
  ...
  function iniciarReconocimiento() {
    ...
    if (btn) btn.classList.add('grabando'); // 🔹 Cambia el color a verde
    if (ta) {
    ...
    recognition.onend = () => {
      ...
      grabando = false;
      recognition = null;
      if (btn) btn.classList.remove('grabando'); // 🔹 vuelve a rojo
    };
  ...
  function detenerReconocimiento() {
    if (recognition) {
      try { recognition.abort(); } catch(e) {} // ⚡ detiene inmediatamente
      recognition = null;
    }
    grabando = false;
