 ...
 * - Limita a $maxLen caracteres
 */
function normalizarTitulo(string $texto, int $maxLen = 56): string {
    $texto = preg_replace('/\s+/u', ' ', trim($texto)); // limpia espacios
    if ($texto === '') return '';

    // Primera letra mayúscula, resto igual
    $primera = mb_strtoupper(mb_substr($texto, 0, 1, 'UTF-8'));
...
$tituloCarpeta = normalizarTitulo((string)($_POST['tituloCarpeta'] ?? ''));  // ✅ ESTA LÍNEA FALTABA
