...
error_log("[LOGIN_ACCESS_DEBUG] Host: " . ($_SERVER['HTTP_HOST'] ?? 'UNKNOWN') . " | Method: " . $_SERVER['REQUEST_METHOD'] . " | IP: " . ($_SERVER['REMOTE_ADDR'] ?? 'UNKNOWN') . " | Session ID: " . session_id() . " | Cookies: " . json_encode($_COOKIE) . " | CSRF in Session: " . ($_SESSION['csrf_token'] ?? 'NONE'));
